...
class UserInformation:
    ...
    def create_bearer_token(self, **data):
        ...
        refresh_jwt_data = dict(user_id=data.get("user_id"), timestamp=str(datetime.now()))
        ...
        # set of JWT Tokens.
        self._save_bearer_token(
            user_id=data.get("user_id"), bearer_token=bearer_token.__dict__
        )
    ...
    def refresh_token(self, refresh_token: str) -> dict:
        ...
        decoded_refresh = jwt.decode(refresh_token, SECRET, ALGORITHM)
        ref_token_data = dict(user_id=decoded_refresh['user_id'],timestamp=str(datetime.now()))
        # Access token data
    ...
    def _revoke_oauth_code(self, code):
    ...
    def _save_user_record(self, user_data):
        # Read file step
        file_path = self.basedir + self.user_info_path
        try:
            _file = open(file_path, "rb")
