...
from sqlite3 import OperationalError, IntegrityError, InterfaceError
import sqlite3
import logging
import os
...
class MainDB:
    ...
    def _create_all(self, database):
        ...
        self._create_mn_info_table(database)
        self._create_device_context_table(database)
    ...
    @staticmethod
    def _create_mn_info_table(database: object) -> None:
        mn_info_query = \
            'CREATE TABLE IF NOT EXISTS ' + \
            'MN_INFO(' + \
            'id INTEGER PRIMARY KEY,' + \
        ...
        cursor = database.cursor()
        cursor.execute(mn_info_query)

    @staticmethod
    def _create_device_context_table(database) -> None:
        device_context_query = \
            ...
            'FOREIGN KEY (device_id) REFERENCES DEVICE_INFO(id))'
        cursor = database.cursor()
        cursor.execute(device_context_query)
    ...
    def init_session(self, query, *args):
        ...
        except (OperationalError, IntegrityError, InterfaceError) as e:
...
#############################################################
    ...
    def _devices_mock_inserts(database):
        ...
        cursor.execute('DELETE FROM MN_INFO')
        cursor.execute('DELETE FROM DEVICE_CONTEXT')
        for i in range(1, 5000):
            # Devices query
            q_device = \
                'INSERT INTO DEVICE_INFO' + \
                '(id,user_id,label,device_handler) ' + \
                'VALUES ' + \
                '(?,?,?,"c2c-switch")'
            cursor.execute(q_device, (f'x{i}', i, f'device_label_{randint(1,1000)}'))
            # Poll query
            q_poll = \
            ...
            mn_query = \
                'INSERT INTO MN_INFO' + \
                '(device_id,manufacturer_name,model_name,hw_version,sw_version) ' + \
                'VALUES ' + \
                '("%s","MN","MODEL","HW_VERSION","SW_VERSION")' % f'x{i}'
            cursor.execute(mn_query)
            ctx_query = \
                'INSERT INTO DEVICE_CONTEXT' + \
                '(device_id,categories,groups,room_name) ' + \
                'VALUES ' + \
                '("%s","light,switch","office,lighting","office")' % f'x{i}'
            cursor.execute(ctx_query)
